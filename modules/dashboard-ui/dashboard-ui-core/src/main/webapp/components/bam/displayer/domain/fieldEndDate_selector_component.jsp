...
%>
    <td align="left">
        <select name="idEndDateDetails" title="<%= endDateProperty.getName(locale) %>" id="<factory:encode name="idEndDateDetails"/>" class="skn-input"
                ...
                >
        <%
            List<DataProperty> endDateProperties = Arrays.asList(displayer.getDomainPropertiesAvailable().clone());
            DataDisplayerServices.lookup().getDataProviderManager().sortDataPropertiesByName(endDateProperties, true);

            for (DataProperty dataProperty : endDateProperties) {
                String selected = "";
                if (dataProperty.getPropertyId().equals(endDateProperty.getPropertyId())) selected = "selected";
        %>
