...
%>
    <td align="left">
        <select name="idProgressDetails" title="<%= progressProperty.getName(locale) %>" id="<factory:encode name="idProgressDetails"/>" class="skn-input"
                ...
                >
        <%
            List<DataProperty> progressProperties = Arrays.asList(displayer.getDomainPropertiesAvailable().clone());
            DataDisplayerServices.lookup().getDataProviderManager().sortDataPropertiesByName(progressProperties, true);

            for (DataProperty dataProperty : progressProperties) {
                String selected = "";
                if (dataProperty.getPropertyId().equals(progressProperty.getPropertyId())) selected = "selected";
        %>
