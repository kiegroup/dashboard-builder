...
%>
    <td align="left">
        <select name="idSizeDetails" title="<%= sizeProperty.getName(locale) %>" id="<factory:encode name="idSizeDetails"/>" class="skn-input"
                ...
                >
        <%
            List<DataProperty> sizeProperties = Arrays.asList(displayer.getDomainPropertiesAvailable().clone());
            DataDisplayerServices.lookup().getDataProviderManager().sortDataPropertiesByName(sizeProperties, true);

            for (DataProperty dataProperty : sizeProperties) {
                String selected = "";
                if (dataProperty.getPropertyId().equals(sizeProperty.getPropertyId())) selected = "selected";
        %>
