...
%>
    <td align="left">
        <select name="idStartDateDetails" title="<%= startDateProperty.getName(locale) %>" id="<factory:encode name="idStartDateDetails"/>" class="skn-input"
                ...
                >
        <%
            List<DataProperty> startDateProperties = Arrays.asList(displayer.getDomainPropertiesAvailable().clone());
            DataDisplayerServices.lookup().getDataProviderManager().sortDataPropertiesByName(startDateProperties, true);

            for (DataProperty dataProperty : startDateProperties) {
                String selected = "";
                if (dataProperty.getPropertyId().equals(startDateProperty.getPropertyId())) selected = "selected";
        %>
