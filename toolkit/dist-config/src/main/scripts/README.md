...
2) How too use the tool.
    ...
    script option: one of the following values: jboss, jetty, factory.

    jboss: indicates that the distribution will be configured for the JBoss application server.

    jetty: indicates that the distribution will be configured for the Jetty server.

    factory: indicates that only application components will be configured. NOT including web application descriptors
             as web.xml, jboss-web.xml, etc.

    product: a path to the product distribution file you want to configure. e.g. ./product/dashbuilder-jboss-as7.0.war

    config: a path to a java property file with the configuration parameters for the product components and
