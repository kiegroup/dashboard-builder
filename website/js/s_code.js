...
function s_doPlugins(s) {
	...
	if(!s.campaign && s.getQueryParam('sc_cid'))
	{
		s.campaign=s.getQueryParam('sc_cid');
	}
	/* Internal Campaign Tracking */
	if(!s.eVar1 && s.getQueryParam('intcmp'))
	{
	...
	s.setCookieParam('rh_omni_tc', s.getQueryParam('sc_cid'),1,30);

	s.setCookieParam('rh_omni_itc',s.getQueryParam('intcmp'),1);

	// set offer cookie when offer_id is present in QS
	var offer = s.getQueryParam(OFFER_PARAM);
	...
	if((document.URL.indexOf("/training/") !== -1) || (document.URL.indexOf("/resourcelibrary/") !== -1)) {
		if(!s.eVar34 && s.getQueryParam('cr'))
		{
			s.eVar34=s.prop34=s.getQueryParam('cr');
		}
	}
}
...
if(!getCookie('rh_pid')){
    var var_pid = s.getQueryParam('pid');
    if(var_pid){        
...
s.Integrate.onLoad=function(s,m){
  ...
  /*
   * [Begin] Partner Plugin: Demandbase v2.0.1 im
   */
